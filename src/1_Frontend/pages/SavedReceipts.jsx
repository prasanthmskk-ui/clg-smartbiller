  ...
  React.useEffect(() => {
    const fetchReceipts = async () => {
      ...
      try {
        ...
        // Use the actual server response whenever the API responds.
        setReceipts(Array.isArray(data) ? data : [])
      } catch (err) {
        console.warn(
          'Server unavailable; loading receipts from LocalStorage:',
          err?.message
        )
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredReceipts.map((receipt) => {
              ...
              const customerName =
                receipt.customer_name?.trim() || 'Guest'
              const customerPhone =
